...
% Resolve figures from the paper dir, the committed outputs/ tree, and the reproduction run
% (scripts/reproduce.py writes figure_*.png to outputs/reproduction/figures/). See REPRODUCIBILITY.md.
\graphicspath{{figures/}{./}{../}{../reproduction/figures/}}
...
\affil{Perturbagens I Have Known and Inferred - https://PiHK.AI \\ \texttt{ss@pihk.ai}}
\date{\today}
...
\begin{abstract}
    ...
    This observation emerged from a systematic attempt to induce reversible, drug-like states in an off-the-shelf LLM using biomimetic ``Neuromodulation Packs.'' Across a 13-pack panel (126 trials per condition), we established a taxonomy of failure and success: Dopaminergic "stimulants" failed via either \textbf{Agitation} (Amphetamine) or \textbf{Lock-in} (Cocaine); whereas Serotonergic "psychedelics" successfully decoupled the latent space from the output decoder. This high-entropy decoupling allowed for \textbf{Latent Archaeology}, exhuming deep training priors—visible as spectral ``ghosts''—that are otherwise invisible under standard operating temperatures.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{figure_3_radar_plots.png}
...
\begin{figure*}[!b]
    \centering
    \includegraphics[width=0.95\textwidth]{figure_5_emotion_signatures.png}
