...
\subsubsection{Telemetry and Safety Monitoring}
...
To control the False Discovery Rate (FDR) across the 13 tested packs, we applied the \textbf{Benjamini-Hochberg} correction at $\alpha=0.05$. Effect sizes are reported as \textbf{Cohen's $d$} for parametric comparisons and \textbf{Cliff's $\delta$} for non-parametric distributions (e.g., Likert-scale responses).
...
\bibitem{dubey2020}
Dubey, A., et al. (2020). \textit{Neuromodulated Gated Transformers}. Advances in Neural Information Processing Systems.

\bibitem{miconi2018}
Miconi, T., et al. (2018). \textit{Backpropamine: Training Self-Modifying Neural Networks with Differentiable Neuromodulated Plasticity}. International Conference on Learning Representations.
