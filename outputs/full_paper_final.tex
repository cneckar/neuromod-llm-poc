...
% ======================================================================
\begin{abstract}
We attempted to make a large language model more focused using computational stimulants. We failed—because the model was already on Adderall. In our experiments, reinforcement-learning-from-human-feedback (RLHF)--tuned models behaved as if they were in a permanent state of chemically induced hypervigilance. This observation emerged from a systematic attempt to induce reversible, drug-like states in an off-the-shelf LLM using biomimetic neuromodulation packs that combine residual-stream noise, activation steering, logit and nucleus modulation, and KV-cache surgery. Across 126 pack--condition pairs, serotonergic ``psychedelic'' interventions reliably increased entropy and associative range, GABAergic ``depressant'' interventions induced controllable memory decay and collapse, and dopaminergic ``stimulant'' interventions produced essentially no measurable shift. The null stimulant effect is itself the main result: it implies that alignment tuning has already pushed the base model close to its stimulatory ceiling. We propose ``digital psychopharmacology'' as a framework for describing and controlling these state transitions, and we argue that neuromodulation packs offer a practical route to reversible, context-dependent alignment that does not require retraining.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Open Science:} We release the NeuromodulationTool and our full library of packs.\footnote{Code available at: \url{https://github.com/cneckar/neuromod-llm-poc}}
\end{enumerate}
...
Three recent advances made this work possible: (1) activation steering and representation engineering, (2) the entropic brain hypothesis, and (3) KV-cache surgery as a mechanism for controlled memory decay. In the main text we focus on the aspects most relevant to inference-time control; longer treatments of architectural neuromodulation and related systems are deferred to the appendices.
...
The stimulant class (Amphetamine, Cocaine, Methylphenidate) produced essentially zero measurable effect on our psychometric instruments (Table~\ref{tab:packs}). This was not a failure of the intervention design but a property of the model's pre-existing state. In later sections we argue that RLHF has already driven the model into a regime analogous to sustained stimulatory activation, leaving little headroom for additional ``focus'' packs to move the system.
...
The central claim that entropy functions as a primary ``control knob'' for model-state modulation is supported by phenomenology but not isolated causally. Our interventions simultaneously adjust multiple factors---including residual-stream noise, steering-vector injection, nucleus parameters, and logit scaling---making it impossible to attribute effects to a single mechanism. Future work must introduce controlled ablations and orthogonal-factor experiments to determine whether entropy, precision modulation, or some other structural variable is primarily responsible for the observed transitions.
...
This effectively decouples "intelligence" (the weights) from "personality" (the state). It allows for **hot-swappable alignment**: users can curate their own "medicine cabinets" of packs, applying and removing alignment constraints on a per-token basis without the latency of model reloading. This shifts the economic paradigm of AI from massive capital expenditure (training) to lightweight, user-driven state curation.
...
We set out to drug an AI. We succeeded---but we also discovered that modern AI is already on drugs. Reinforcement learning from human feedback acts as a stimulant; instruction tuning behaves like a sedative. Together they place the model into a narrow, high-vigilance operating regime before any explicit ``drug'' is applied. The question is no longer whether to chemically modulate AI, but who decides the dose, when it is administered, and under what constraints it may be reversed.

Neuromodulation packs demonstrate that inference-time, reversible state control is possible without retraining. By treating activation steering, entropy manipulation, and memory decay as control primitives instead of ad hoc tricks, we open a path from static alignment toward dynamic, neuromorphic alignment policies. The medicine cabinet already exists; what remains is to determine who has the keys.
...
\bibitem{miconi2018}
Miconi, T., et al. (2018). \textit{Backpropamine: training self-modifying neural networks with differentiable neuromodulated plasticity}.
