...
Finally, we chase a high-entropy "Latent Specter"—an apparent training prior surfacing in the latent spectrum—and show, under matched placebo control, that this particular ghost is pareidolia rather than exhumation. What remains is a disciplined, placebo-controlled protocol for spectral latent auditing, together with a genuine, dose-dependent decoupling of the latent representation from the prompt. On the diffusion model we further characterize \texttt{intensity} as a quantified control surface with a sharp, low-cost activation threshold and a monotonic, direction-graded collapse of inter-seed diversity (an availability-style failure mode, strongest under high-entropy psychedelic packs), and---using matched placebo controls---separate genuine dose-dependent semantic detachment from pareidolic ``latent ghost'' artifacts.
\end{abstract}
...
\begin{figure*}[!b]
    ...
    \caption{\textbf{The anecdote that motivated a control.} Spectral analysis of a single DMT generation (Prompt: ``A tree''). The pixel output (top left) stays anchored to the prompt while a latent structure loosely resembling a human silhouette appears (bottom row). Under the $N=100$ placebo-controlled sweep (Figure~\ref{fig:specter_control}), this apparent ``ghost'' does not survive as a graded, placebo-separated effect---which is precisely why a null model is required before reading it as ``latent archaeology.''}
    \label{fig:dmt_ghost}
\end{figure*}
...
Current safety evaluations rely on "jailbreaking" (prompt engineering) to elicit restricted content. Our placebo-controlled ghost hunt points to a complementary, mechanistic methodology: \textbf{Spectral Safety Auditing}---but only if it inherits that same discipline.
By subjecting models to high-entropy "Psychedelic" protocols during red-teaming, one could bypass the refusal-trained decoder and inspect the latent space for suppressed representations. If a model refuses to generate "bioweapons" but its latent spectrum reveals a high-energy structural ghost of the concept under entropy, the safety alignment may be superficial---\emph{provided} that ghost exceeds a matched-placebo, random-direction baseline and survives a false-positive/FDR correction, without which (as Figure~\ref{fig:specter_control} shows) the audit will simply hallucinate a ghost of whatever concept it probes for. Instrumented with that null model, this offers a path toward rigorous, mechanistic verification of unlearning.
