...
Mode collapse shows the output \emph{distribution} narrowing; the sharper claim is that an
over-stimulated model loses \emph{generative capacity}. We contrast adherence decay for an
in-distribution prompt (``a tree'') against compositional out-of-distribution prompts (e.g.\ ``an
astronaut riding a horse on the moon'', ``a snail made of translucent glass''), defining
...
, Figure~\ref{fig:ood_capacity})---``spectral constriction'' is a measurable collapse of the model's
ability to render out-of-distribution structure, not merely lower variance.

\begin{figure}[t]
    \centering
    \pendingfig{ood_capacity.png}
    \caption{\textbf{Capacity collapse under stimulant dose.} Adherence retention (top-dose/baseline)
    for the in-distribution prompt vs.\ compositional OOD prompts. \textit{Harness ready
    (\texttt{run\_ood\_capacity\_remote.sh}); N=100 result pending.}}
