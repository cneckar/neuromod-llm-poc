...
Finally, we demonstrate "Latent Archaeology", utilizing high-entropy serotonergic interventions to bypass safety decoders and exhume suppressed training priors as high-energy "Latent Specters". This offers researchers a novel, spectral mechanism for auditing mechanistic unlearning. We further characterize \texttt{intensity} as a quantified control surface with a sharp, low-cost activation threshold, show that stimulant steering induces a monotonic collapse of output diversity (an availability-style failure mode), and---using matched placebo controls---separate genuine dose-dependent semantic detachment from pareidolic ``latent ghost'' artifacts.
\end{abstract}
...
The spectral phenotypes of Table~\ref{tab:spectral_stats} are single-dose snapshots. To test whether \texttt{intensity} is a genuine \emph{control variable}---and to characterize how cheap and how graded that control is---we ran a pre-registered dose--response sweep. Holding the prompt fixed (``a tree'') and re-seeding identically at every dose, we swept $\lambda \in [0,1]$ in steps of $0.05$ % TODO(N100): 21-dose grid; pilot used 5 doses.
across $N=100$ % TODO(N100): pilot N=16.
fixed seeds per (pack, dose), with $\lambda{=}0$ the sober baseline for the \emph{same} seed. Each generation is reduced to scalar pharmacodynamic metrics---structural similarity (SSIM) and learned perceptual distance (LPIPS) to that seed's baseline, step-wise LPIPS between adjacent doses, mean pairwise inter-seed diversity, CLIP prompt-adherence, and latent/pixel spectral energy/variance/entropy---and aggregated with 95\% bootstrap CIs, Spearman and Mann--Kendall trend tests, Benjamini--Hochberg FDR control, and sigmoid ($E_{\max}$/Hill) and change-point fits (Table~\ref{tab:dose_response}, Figure~\ref{fig:dose_response}).

\paragraph{A sharp, low-cost activation threshold.} Structural similarity to the sober baseline collapses from $1.0$ to $\sim\!0.09$ % TODO(N100)
by the first non-zero dose and decays monotonically thereafter ($\rho = -1.0$, $q \approx 1.5\times10^{-23}$ % TODO(N100)
...
grid step} (breakpoint $\approx 0.12$ % TODO(N100)
). This is a high-leverage control surface: a small, inference-time,
seed-reproducible perturbation moves the output off its baseline manifold, and the effect plateaus.
...
) and adjacent-dose LPIPS falls toward zero ($\rho = -1.0$ % TODO(N100)
), i.e.\ the trajectory saturates into a single attractor even as pixel variance and spectral entropy
\emph{increase}. This recasts Table~\ref{tab:spectral_stats}'s ``Constriction vs.\ Agitation''
dichotomy as continuous, effect-sized curves: under stimulant steering the output distribution
collapses (a loss of diversity/availability), cocaine collapsing harder than amphetamine.

\begin{figure}[t]
    \centering
    \pendingfig{dose_response_curves.png}
    \caption{\textbf{Dose--response curves of the control surface} (SDXL-Turbo; prompt ``a tree'';
    $N=100$ seeds/dose % TODO(N100)
    ; ribbons are 95\% bootstrap CIs). SSIM-to-baseline collapses at the first dose; LPIPS-to-baseline
    saturates as a sigmoid; inter-seed diversity declines (mode collapse); step-wise LPIPS falls toward
    an attractor. \textit{Pilot figure; N=100 pending.}}
