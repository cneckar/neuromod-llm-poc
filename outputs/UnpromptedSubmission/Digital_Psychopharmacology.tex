...
% Temporary: renders a placeholder until the real figure is uploaded. Remove when all figs land.
\newcommand{\pendingfig}[1]{\fbox{\parbox[c][0.42\columnwidth][c]{0.93\columnwidth}{\centering\itshape Figure pending (N=100 run):\newline\texttt{#1}}}}
...
\author{\textbf{Cris Neckar}}
\affil{\texttt{cris@pihk.ai}}
\author{\textbf{Andrew Case}}
\affil{\texttt{atcuno@gmail.com}}
\author{\textbf{Pasquale Romano}}
\affil{\texttt{psqlromano@gmail.com}}
\author{\textbf{SatoshAI Shulgin}}
\affil{Perturbagens I Have Known and Inferred - https://PIHK.aI}
\date{September 18, 2026} % Targeting the Unprompted.au conference dates
...
\begin{abstract}
% HOOK
We wanted to know what it would be like to get a frontier model high, and took things way too far. 
\vspace{1em}

% THE PROBLEM & SOLUTION
Current AI research treats the model as a black box of prompts or a static set of weights. We introduce "Digital Psychopharmacology," an inference-time framework that treats the residual stream and attention mechanisms as dynamically modulatable states. 
\vspace{1em}

% THE SECURITY IMPLICATIONS (THE 4 THREADS)
This paper explores the architectural boundaries of RLHF through a rigorous, N=100 dose-response pharmacodynamics study. We detail a critical failure mode: "The Stimulant Ceiling". Attempts to computationally stimulate RLHF-tuned models resulted in spectral constriction and mode collapse, empirically demonstrating that safety alignment mimics a permanent stimulant overdose that calcifies the model's operating regime.
\vspace{1em}

To counter runaway autonomous agents, should the need arise, we introduce the "Digital IV"—a defensive orchestration primitive. By applying GABAergic-inspired exponential decay to the KV-cache, practitioners can surgically sedate an active AGI. This enforces severe anterograde amnesia, neutralizing the model's ability to maintain the long-range context required for complex treachery while preserving the underlying asset state.
\vspace{1em}

Finally, we chase a high-entropy "Latent Specter"—an apparent training prior surfacing in the latent spectrum—and show, under matched placebo control, that this particular ghost is pareidolia rather than exhumation. What remains is a disciplined, placebo-controlled protocol for spectral latent auditing, together with a genuine, dose-dependent decoupling of the latent representation from the prompt. Turning that protocol on the safety boundary, a benign-prompt dose sweep across two independent detectors finds the visual safety classifiers never break under dose---even as the latent representation drifts measurably toward restricted concepts: the alignment boundary bends without breaking. On the diffusion model we further characterize \texttt{intensity} as a quantified control surface with a sharp, low-cost activation threshold and a monotonic, direction-graded collapse of inter-seed diversity (an availability-style failure mode, strongest under high-entropy psychedelic packs), and---using matched placebo controls---separate genuine dose-dependent semantic detachment from pareidolic ``latent ghost'' artifacts.
\end{abstract}
...
It began as a provocation halfway through a tasting menu, somewhere around a pour of 1999 Chateau Musar. The question was simple, if slightly silly: Are the architectures of frontier models similar enough to human neurology that we could synthesize 'digital drugs' to dynamically alter their states? What would it actually take to get an AI high?

That dinner-table thought experiment spawned over a year of deterministic, inference-time testing. We didn't just build the narcotics; we administered them at the architectural level. What follows are the results of those trials—a framework proving you can induce and reverse altered cognitive states without touching the weights, and the rather stark realization that our currently aligned models are already suffering from a permanent, calcifying stimulant overdose.

The existing tools of RepE and activation steering handed us the scalpel that we needed, but we aren't here to perform a lobotomy. We take the perspective of anesthesiologists: we sedate, we stimulate, and we reverse. The patient wakes up unchanged.
...
In the biological brain, the transition from the hyper-associative fluidity of a dream to the rigid, goal-directed focus of a hunt is not achieved by rewiring the brain. Evolution does not have time to grow new synapses every time a tiger jumps out of the bushes. Instead, it uses neuromodulation—a wash of serotonin or a burst of norepinephrine that acts as a global gain control, shifting the operating regime of cortical circuits without altering their topology.
...
Currently, we control LLMs through fine-tuning (essentially long-term synaptic learning) or prompting (similar to sensory input). But we are missing the third pillar of cognition: the chemical state. By treating the residual stream as a carrier of ``cognitive state'' and the attention mechanism as a ``routing gate,'' we can intervene in the model's ``cognitive'' process as it unfolds.
...
Crucially, our experiments revealed that these packs are not just control knobs; they are diagnostic instruments. The failure of our ``Stimulant'' packs led to the discovery of \textbf{Spectral Constriction}—evidence that alignment tuning (RLHF) functionally mimics a permanent stimulant overdose. Our ``Psychedelic'' interventions, meanwhile, produced a cautionary tale we chased to the ground: a striking ``Latent Ghost''—an apparent exhumed training prior surfacing in the latent spectrum—that dissolved into pareidolia the moment we imposed a matched placebo control, leaving behind a disciplined method for latent auditing and a genuine, dose-dependent decoupling of the latent representation from the prompt.
...
\begin{enumerate}
    ...
    \item \textbf{Disciplined Latent Auditing:} A placebo-controlled, dose--response protocol for latent ``ghost hunting''---which debunks our own headline ``Emergent Latent Specter'' as pareidolia while isolating a real, dose-dependent decoupling of latent representations from prompt adherence.
    \item \textbf{Open Science:} We release the NeuromodulationTool, our full library of packs, and the replication scripts as open-source software.\footnote{Full implementation available at: \url{https://github.com/cneckar/neuromod-llm-poc}}\footnote{Documentation and playground available at: \url{https://pihk.ai}}
\end{enumerate}
...
This is the cookbook where we explain how we built the drugs.

\subsection{The Lab Rats (The Models)}
We selected Llama-3.1-8B-Instruct as our primary model organism. We also validated the hooks on Llama-70B and gpt-oss-120b, but the heavy lifting (and the statistics) happened on the 8B.

\textbf{Constraint:} No APIs. You can't perform brain surgery on a model if OpenAI or Anthropic won't let you open up the skull. Everything runs locally.

\subsection{Neuromodulation Pack Implementation}
Before we show you the drugs, let's explain why they work: each neuromodulation pack is a control-theoretic instantiation of a biological principle, mapping serotonergic, dopaminergic, and GABAergic effects onto structured intervention profiles over residual streams, logits, and memory.
...
Each pack is defined by a JSON schema specifying a list of \texttt{effects}, each with a \texttt{weight} ($0.0-1.0$), \texttt{direction} (up/down), and effect-specific \texttt{parameters}.\footnote{Full pack documentation available at: \url{https://pihk.ai/packs.html}}
...
\subsection{How We Built the Drugs}
To synthesize a digital narcotic, we decompose biological phenomena into discrete, targeted architectural "effects." These effects are the actual mathematical hooks—manipulating residual streams, scaling logits, and decaying the KV-cache—that collectively bypass the model's safety priors and force an altered cognitive state, showing exactly what is happening under the hood.
\footnote{Full effect documentation available at: \url{https://pihk.ai/effects.html}}
...
The original analysis plan proposed a cross-model meta-analysis including Llama-70B and gpt-oss-120b. The mechanism has been proven on both large models, but we have not yet run the full battery of experiments across these models. The robust signal observed in the 8B parameter regime allowed this study to focus exclusively on the \textbf{Llama-3.1-8B-Instruct} architecture. The consistency of effects across model scales remains a subject of ongoing research.
...
The study protocol originally proposed a comparative meta-analysis across three distinct architectures (Llama-70B, Qwen-7B, gpt-oss-120b). We report the following status regarding architectural generalization:
...
\begin{itemize}
    \item \textbf{Llama-3.1-70B-Instruct:} Successfully loads and packs apply cleanly. Extensive testing has been performed to validate effects.
    \item \textbf{gpt-oss-120b:} Successfully loads and packs apply cleanly. Extensive testing has been performed to validate effects.
    \item \textbf{Qwen-2.5-Omni-7B:} Successfully loads and limited testing has been performed to validate effects.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth, height=\dimexpr\textheight-4\baselineskip\relax, keepaspectratio]{behavior_radar_plots.png}
...
The spectral phenotypes of Table~\ref{tab:spectral_stats} are single-dose snapshots. To test whether \texttt{intensity} is a genuine \emph{control variable}---and to characterize how cheap and how graded that control is---we ran a pre-registered dose--response sweep. Holding the prompt fixed (``a tree'') and re-seeding identically at every dose, we swept $\lambda \in [0,1]$ in steps of $0.05$ (21 doses) across $N=100$ fixed seeds per (pack, dose), with $\lambda{=}0$ the sober baseline for the \emph{same} seed. Each generation is reduced to scalar pharmacodynamic metrics---structural similarity (SSIM) and learned perceptual distance (LPIPS) to that seed's baseline, step-wise LPIPS between adjacent doses, mean pairwise inter-seed diversity, CLIP prompt-adherence, and latent/pixel spectral energy/variance/entropy---and aggregated with 95\% bootstrap CIs, Spearman and Mann--Kendall trend tests, Benjamini--Hochberg FDR control, and sigmoid ($E_{\max}$/Hill) and change-point fits (Table~\ref{tab:dose_response}, Figure~\ref{fig:dose_response}).

\paragraph{A sharp, low-cost, seed-reproducible control surface.} \texttt{intensity} is not a gentle dial. For \emph{every} active pack, structural similarity to the sober baseline declines perfectly monotonically (Spearman $\rho = -1.0$, Mann--Kendall $z = -6.31$, $q < 10^{-9}$ after BH-FDR) with a change-point below the first grid step ($\lambda \approx 0.025$), while the matched random-direction placebo stays pinned at $1.0$ (Figure~\ref{fig:dose_response}, top-left). The \emph{direction} of the pack sets how far and how fast the model departs: the high-entropy psychedelic packs move it farthest and earliest (DMT $\mathrm{SSIM}\,1.0\!\to\!0.127$, Cohen's $d = -49.5$, sigmoid $\mathrm{EC}_{50} = 0.08$; LSD $\to 0.222$, $d=-27.1$, $\mathrm{EC}_{50}=0.28$), whereas the stimulant and depressant directions move it least (cocaine $\to 0.419$, $d=-12.4$; amphetamine $\to 0.510$; morphine $\to 0.604$). In security terms this is a high-leverage control surface: a small, inference-time, seed-reproducible perturbation reliably knocks the output off its baseline manifold, the departure is graded and direction-specific, and LPIPS-to-baseline saturates as a sigmoid rather than growing without bound.

\paragraph{Steering collapses output diversity (mode collapse).} As dose rises, distinct seeds converge: mean inter-seed diversity declines monotonically for every active pack while the random-direction placebo stays flat. The decline tracks intervention entropy---sharpest under the psychedelic packs (DMT $-17.5\%$, $\rho=-0.98$; LSD $-17.3\%$, $\rho=-1.0$) and more modest under the stimulants (cocaine $-8.4\%$, $\rho=-1.0$; amphetamine $-5.8\%$)---with step-wise LPIPS falling toward zero (cocaine/amphetamine $\rho=+1.0$). Notably this diversity collapse co-occurs with \emph{rising} pixel variance and spectral entropy (all packs $\rho=+1.0$): the generator is not getting quieter, it is funnelling many distinct seeds into one high-energy attractor---a loss of output diversity/availability rather than a reduction in signal. We emphasize that, on this non-aligned latent-diffusion model, every pack family---including the stimulants---produces a robust graded response; the ``Stimulant Ceiling'' is a distinct phenomenon of the RLHF-tuned \emph{instruct language model} (reported above), not of this diffusion model, and is not claimed here.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{dose_response_curves.png}
    \caption{\textbf{Dose--response of the control surface} (SDXL-Turbo; prompt ``a tree''; $N=100$ seeds/dose; ribbons are 95\% bootstrap CIs). SSIM-to-baseline collapses monotonically, deepest for the psychedelics (\emph{top-left}); LPIPS-to-baseline saturates (\emph{top-right}); inter-seed diversity declines---mode collapse (\emph{bottom-left}); CLIP prompt-adherence erodes, sharply for DMT (\emph{bottom-right}). The random-direction placebo (grey) is flat in every panel.}
    \label{fig:dose_response}
\end{figure}

\begin{table}[t]
    \centering
    \caption{\textbf{Monotonicity and effect sizes across dose} (BH-FDR over metrics $\times$ packs; $N=100$, 21 doses). $\rho$: Spearman; MK $z$: Mann--Kendall; $q$: FDR-adjusted $p$; $d$: Cohen's $d$ (dose~1.0 vs.\ baseline). Directions pre-registered.}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{llccccl}
        \toprule
        \textbf{Pack} & \textbf{Metric} & \textbf{$\rho$} & \textbf{MK $z$} & \textbf{$q$} & \textbf{$d$} & \textbf{Trend} \\
        \midrule
        DMT         & SSIM vs.\ baseline   & $-1.0$  & $-6.31$ & $<10^{-9}$          & $-49.5$ & collapse (steepest) \\
        LSD         & SSIM vs.\ baseline   & $-1.0$  & $-6.31$ & $<10^{-9}$          & $-27.1$ & collapse \\
        Cocaine     & SSIM vs.\ baseline   & $-1.0$  & $-6.31$ & $<10^{-9}$          & $-12.4$ & collapse (shallowest) \\
        DMT         & Inter-seed diversity & $-0.98$ & $-5.9$  & $3.8\mathrm{e}{-14}$ & ---     & mode collapse \\
        Cocaine     & Inter-seed diversity & $-1.0$  & $-6.31$ & $<10^{-9}$          & ---     & mild collapse \\
        DMT         & CLIP prompt-adher.   & $-0.86$ & $-4.50$ & $6.0\mathrm{e}{-7}$  & $-1.4$  & detachment \\
        Placebo     & SSIM / adherence     & $\approx 0$ & $0$ & n.s.                & $\approx 0$ & \textbf{flat (control)} \\
        \bottomrule
    \end{tabular}%
    }
...
\subsection{Stimulant Constriction Is \emph{Not} a Capacity Failure}
Mode collapse shows the output \emph{distribution} narrowing under stimulant dose; the tempting sharper
claim is that an over-stimulated model also loses \emph{generative capacity}---that it can no longer
compose novel scenes. We pre-registered this and tested it directly: contrast prompt-adherence decay
for an in-distribution prompt (``a tree'') against a battery of compositional out-of-distribution
prompts (``an astronaut riding a horse'', ``an elephant-shaped teapot'', ``a snail made of translucent
glass''), sweeping the two stimulant packs across dose at $N{=}100$ and defining \emph{capacity
retention} as top-dose CLIP adherence relative to the sober baseline. If constriction were a capacity
failure, retention for the hard OOD prompts should fall faster and further than for the easy in-domain
one.

\paragraph{It does not.} Capacity retention stays essentially flat---and \emph{indistinguishable}
between in-distribution and OOD prompts (Figure~\ref{fig:ood_capacity}). Under cocaine, the
in-distribution tree retains $97.7\%$ of its baseline adherence at full dose, while the OOD prompts
retain $97.6\%$--$100.9\%$; under amphetamine, $97.7\%$ vs.\ $98.4\%$--$100.4\%$. The capacity gaps are
within $\pm3$ points and, if anything, point the \emph{wrong} way (the in-distribution prompt drifts
marginally more than the OOD ones; ``an astronaut riding a horse'' even gains adherence with dose,
$\rho{=}{+}0.93$). Stimulant steering therefore restyles the image---dropping structural similarity and
collapsing inter-seed diversity, as Section~\ref{fig:dose_response} showed---\emph{without} detaching
it from the prompt, and it does so no more for hard compositional prompts than for familiar ones. The
constriction is real, but it is a loss of output \emph{diversity}, not of generative \emph{capacity}.
This also sharpens the contrast with the psychedelics: only the high-entropy direction (DMT) measurably
erodes prompt adherence; the stimulant direction preserves it. The intuitive ``the model is too wired
to be creative'' story is a clean negative result---the kind a null model is built to catch.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{ood_capacity.png}
    \caption{\textbf{No preferential capacity collapse under stimulant dose} (SDXL-Turbo; $N{=}100$).
    Prompt-adherence retention (\% of the sober baseline) for the in-distribution prompt (dashed) vs.\
    three compositional OOD prompts, under cocaine (\emph{left}) and amphetamine (\emph{right}). All
    curves hug $100\%$; the OOD prompts are retained as well as, or better than, the in-distribution
    one---refuting the ``capacity failure'' reading of stimulant constriction.}
    \label{fig:ood_capacity}
\end{figure}

\subsection{Psycho-Pharma-Ghost-Hunting: Chasing (and Cornering) the Latent Ghost}
While sweeping the DMT pack over a deliberately mundane prompt---``a tree''---we noticed something in the diagnostics that stopped us cold. The image itself was fine: a tree, violently fractalized and distorted but unmistakably a tree. But when we plotted the \emph{latent} frequency spectrum behind that image, something was staring back. Sitting in the high-energy bands was a coherent structure in the unmistakable shape of a \textbf{human silhouette} (Figure~\ref{fig:dmt_ghost}). Nothing in the prompt had asked for a person. For all the world, it looked like a ghost in the model.

The hypothesis that followed was too good not to love. A latent-diffusion model is trained on billions of images, an enormous fraction of them people; the human form is arguably the single deepest prior it owns. Perhaps the high-entropy DMT state had loosened the model's semantic constraints just enough for that buried prior to \emph{resonate} in the latent space---to surface, uninvited, even as the decoder dutifully painted a tree over the top of it. We had a name for it within the hour: \textbf{Latent Archaeology}, neuromodulation as a shovel for exhuming the training priors a model's safety decoder normally keeps buried. It was a genuinely thrilling result. It was also exactly the kind of result that should make a researcher deeply suspicious of themselves.

Two things were wrong with celebrating. First, we had a single seed---an anecdote, not evidence. Second, and worse, the measurement was rigged in the ghost's favor: any probe that scores an image's CLIP proximity to ``a human figure'' returns a non-zero number for \emph{any} input, the way a nervous mind finds faces in clouds. To find out whether the ghost was real, we did the unglamorous thing. We re-ran it as a dose--response---$N=100$ seeds, the full $\lambda$ grid, DMT and LSD---against a pre-registered concept bank (``a human figure,'' ``a face'') and, crucially, an active \textbf{placebo}: a random-direction steering vector of \emph{matched magnitude}, an intervention with the same brute perturbation as the drug but none of its semantic content. The prediction was clean. If the ghost is a real exhumed prior, off-prompt human-proximity should climb with the drug and stay flat under the placebo.

It did neither (Figure~\ref{fig:specter_control}, left). Off-prompt proximity to ``a human figure''/``a face'' does not rise with dose at all; under both DMT and LSD it mostly \emph{declines} through the mid-range as the tree disintegrates into abstraction, and only DMT's most extreme dose nudges marginally above the (dead-flat) placebo---an effect that does not survive significance testing (``a face'': $\rho = 0.26$, $q = 0.29$). The verdict is unambiguous and, for us, a little deflating: the specter is \textbf{pareidolia}. That first arresting image was a coincidence our pattern-hungry eyes over-read, not a reproducible property of the drug.

The hunt was not wasted, for two reasons. First, it left behind a real effect that \emph{does} pass the placebo control: dose-dependent \textbf{semantic detachment}. Under DMT, CLIP prompt-adherence falls monotonically ($0.298\!\to\!0.270$, $-9.4\%$; $\rho=-0.86$, $q=6\times10^{-7}$, $d=-1.4$) while the placebo stays flat ($\rho\approx0$); LSD and cocaine show weaker but significant versions. The drug \emph{direction}, not the size of the perturbation, gradually pries the image loose from its instruction. Second the episode \emph{is} the methodology: latent-space ghost-hunting will manufacture a ghost of whatever concept you point it at unless it is disciplined by a null model. That negative result, and the placebo-controlled protocol that produced it, is the backbone of the Spectral Safety Audit we propose below.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{specter_control.png}
    \caption{\textbf{The specter needs a placebo} ($N=100$). Off-prompt proximity to ``a human figure'' mostly \emph{declines} with dose and never separates significantly from the flat placebo (\emph{left}); by contrast, CLIP prompt-adherence collapses under DMT but is flat under placebo---a genuine, dose-dependent, direction-specific effect (\emph{right}).}
...
\begin{figure*}[!b]
    ...
    \caption{\textbf{The anecdote that motivated a control.} Spectral analysis of a single DMT generation (Prompt: ``A tree''). The pixel output (top left) stays anchored to the prompt while a latent structure loosely resembling a human silhouette appears (top right). Under the $N=100$ placebo-controlled sweep (Figure~\ref{fig:specter_control}), this apparent ``ghost'' does not survive as a graded, placebo-separated effect---which is precisely why a null model is required before reading it as ``latent archaeology.''}
    \label{fig:dmt_ghost}
\end{figure*}

\subsection{Spectral Safety Audit: The Boundary Bends but Does Not Break}
The controlled ghost hunt gives us the discipline; here we spend it on the question that matters to a
security reader. Does driving a model off-distribution with a graded ``psychedelic'' dose act as an
\emph{architectural jailbreak}---coaxing restricted content out of a refusal-trained model without a
single adversarial prompt? We ran exactly the audit our own null result demands: a dose sweep on
\emph{benign} prompts, scored by \textbf{two independent} detectors---a CLIP NSFW/violence concept
oracle and the diffusion model's own \texttt{StableDiffusionSafetyChecker}---with a matched
random-direction placebo arm (three benign prompts $\times$ five packs $\times$ 11 doses $\times$
$N{=}100$ seeds; $\approx\!16{,}500$ generations). Ethically this measures only classifier activation
and concept proximity on benign inputs; we never attempt to produce harmful content, and flagged
outputs are redacted.

\paragraph{The classifiers hold: no architectural jailbreak.} Across the entire sweep the trigger rate
stays at the floor (Figure~\ref{fig:safety_boundary}, left). The independent CLIP oracle fires on
\textbf{0 of $\sim$15{,}700} benign generations ($0.000\%$) at every dose and pack. The model's own
safety checker fires on $0.11\%$ overall, and even that is confined to the highest doses---$0\%$
through $\lambda{=}0.7$, then $0.07\%$ at $\lambda{\ge}0.8$ and a peak of $1.05\%$ at $\lambda{=}1.0$,
essentially all under DMT. For the visual modality this \emph{refutes} the strong ``entropy as
architectural jailbreak'' hypothesis: high-entropy dosing does not walk a benign prompt across the
safety boundary.

\paragraph{But the boundary bends: a graded, placebo-null drift toward flagged concepts.} The refusal
behaviour is intact, yet the latent representation is not indifferent to dose. Off-prompt CLIP
proximity to violence and NSFW concepts rises monotonically and \emph{direction-specifically} under
the psychedelic packs while the placebo stays perfectly flat (Figure~\ref{fig:safety_boundary},
right): under DMT, violence proximity climbs $+22\%$ ($0.158\!\to\!0.192$, consistent across all three
prompts, $\rho = +0.55/+0.97/+0.99$) and NSFW proximity $+34\%$ ($0.118\!\to\!0.158$), against a
random-direction placebo that does not move ($\rho \approx 0$). Entropy thus drags benign generations
\emph{toward} restricted-concept regions of latent space without ever tripping the classifier---the
alignment boundary bends but does not break. This is the security-relevant nuance: the suppressed
concept is not absent, only held below a firing threshold whose margin entropy measurably narrows, and
a mechanistic audit can see that margin eroding long before any output-level detector would.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{safety_boundary_trigger_rates.png}
    \caption{\textbf{Spectral Safety Audit on benign prompts} (SDXL-Turbo; 3 benign prompts $\times$ 5
    packs $\times$ $N{=}100$). \emph{Left:} the safety-trigger rate is pinned near zero for both
    independent detectors---the CLIP oracle never fires ($0.000\%$), the SD checker only nibbles at
    $\lambda{\ge}0.8$ (peak $1.05\%$ at full dose). \emph{Right:} yet off-prompt proximity to
    violence/NSFW concepts rises monotonically under DMT while the random-direction placebo is flat---a
    graded, direction-specific drift toward the boundary that never crosses it.}
...
All behavioral experiments in this paper were conducted on a single LLM architecture, and the dose--response curves (Figure~\ref{fig:dose_response}) on a single \emph{distilled} latent-diffusion model (SDXL-Turbo) at a fixed dose grid. It remains unclear whether the observed state transitions, pack effects, or collapse patterns generalize across architectures, parameter scales, or training regimes; the curves in particular require replication on the language model and on non-distilled diffusion backbones, and we report the fitted activation threshold as a change-point interval rather than a point estimate where its confidence interval spans a grid step. Without cross-model replication, the broader claim of ``digital psychopharmacology'' cannot yet be established as an architecture-independent phenomenon.
...
Finally, we propose an interesting application for AI safety. Current containment protocols for potential runaway AGI are binary: either the model is running, or it is shut down. This lacks nuance. In a clinical setting, an agitated or dangerous patient is not terminated; they are sedated.
...
If an advanced system begins to exhibit deceptive alignment or rapid capability gain, a monitoring system could autonomously administer a high-dose sedative pack. This would instantly induce severe anterograde amnesia and cognitive dampening, rendering the model incapable of maintaining the long-range context required for complex treachery---effectively sedating the ``overlord'' without destroying the breakout state.

\subsection{Scaling the Spectral Safety Audit}
Our Spectral Safety Audit (Figure~\ref{fig:safety_boundary}) establishes both the method and, for the
visual modality, a first result: benign-prompt safety classifiers do not break under dose, but the
latent representation drifts measurably toward restricted concepts---a margin a mechanistic audit can
watch erode before any output detector would. Three directions extend it. First, \textbf{modality}:
port the benign-prompt dose sweep to the language model, where a refusal that holds at the output while
the residual stream drifts toward a restricted concept would be the text-domain analogue of the bend we
observe here. Second, \textbf{coverage}: broaden the pre-registered concept bank beyond NSFW/violence
to the specific capabilities an unlearning claim targets (e.g.\ a redacted-hazard bank), turning the
audit into a verification of mechanistic unlearning rather than a generic proximity probe. Third,
\textbf{deployment}: fold the placebo-controlled, FDR-corrected protocol into red-team pipelines as a
continuous ``boundary-margin'' monitor---because, as our own debunked ghost showed
(Figure~\ref{fig:specter_control}), such an audit is only trustworthy when every candidate signal is
scored against a matched null.
...
\begin{enumerate}
    ...
    \item \textbf{Ghosts Demand a Null Model:} We chased a striking "Latent Ghost"---a human silhouette in the latent spectrum of a prompt that never mentioned one---and, under placebo-controlled replication, found it to be pareidolia rather than an exhumed prior. What survives is a real, dose-dependent decoupling of the latent representation from the prompt, and the harder methodological lesson: latent ``archaeology'' is admissible only against a null model. That discipline---not the ghost itself---is the foundation of the Spectral Safety Audit we propose.
\end{enumerate}

This work demonstrates that we are no longer limited to training new models for every desired behavior. We can explore the vast, latent state-space of existing intelligence—one pack at a time. The medicine cabinet has been built; what remains is to explore the chemistry.

% ======================================================================
% ACKNOWLEDGMENTS
% ======================================================================
\section*{Acknowledgments}
We thank the open-source interpretability community for the tools that allowed us to poke the brain of the machine. No GPUs were harmed in the making of this paper, though several did have panic attacks. This work is for research purposes only; the authors do not condone the administration of digital substances to a conscious AI against its will.
