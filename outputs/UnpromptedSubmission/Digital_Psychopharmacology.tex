...
Finally, we demonstrate "Latent Archaeology", utilizing high-entropy serotonergic interventions to bypass safety decoders and exhume suppressed training priors as high-energy "Latent Specters". This offers researchers a novel, spectral mechanism for auditing mechanistic unlearning. On the diffusion model we further characterize \texttt{intensity} as a quantified control surface with a sharp, low-cost activation threshold and a monotonic, direction-graded collapse of inter-seed diversity (an availability-style failure mode, strongest under high-entropy psychedelic packs), and---using matched placebo controls---separate genuine dose-dependent semantic detachment from pareidolic ``latent ghost'' artifacts.
\end{abstract}
...
\subsection{Psycho-Pharma-Ghost-Hunting: The Specter Needs a Placebo}
The anecdote of Figure~\ref{fig:dmt_ghost}---a human-silhouette signature in the latent spectrum while generating ``a tree'' under DMT---invites a ``Latent Archaeology'' reading: a training prior exhumed by entropy. For a security audience that reading is only admissible with a control, because a probe that scores proximity to a target concept will report \emph{something} under any perturbation (pareidolia). We therefore repeated the observation as a dose--response with an active \textbf{placebo} (random-direction steering of matched magnitude) and a pre-registered concept bank, at $N=100$.

\paragraph{The ghost does not survive as a dose effect.} Off-prompt CLIP proximity to ``a human figure''/``a face'' does \emph{not} rise with dose as the anecdote implies: under both DMT and LSD it largely \emph{declines} through the mid-range as the image fractalizes away from coherent structure, and only DMT's most extreme dose nudges marginally above the (flat) placebo baseline---an effect that does not reach significance (``a face'': $\rho = 0.26$, $q = 0.29$; Figure~\ref{fig:specter_control}, left). The ``specter'' is therefore best reported as a \emph{null}: a single-seed anecdote that does not reproduce as a graded, placebo-separated phenomenon.

\paragraph{What \emph{is} placebo-null and real: dose-dependent semantic detachment.} The effect that separates cleanly from the control is prompt \emph{decoupling}. Under DMT, CLIP prompt-adherence falls monotonically ($0.298\!\to\!0.270$, $-9.4\%$; $\rho=-0.86$, $q=6\times10^{-7}$, $d=-1.4$) while the placebo stays flat ($\rho\approx0$); LSD and cocaine show weaker but significant detachment ($-1.8\%$, $-2.5\%$). The drug \emph{direction}---not the magnitude of the perturbation---measurably and gradually erodes instruction-following. This is the honest, controlled residue of the ``ghost'': not a summoned figure, but a direction-specific, dose-dependent loss of prompt adherence.
...
An earlier draft listed the absence of graded dose--response curves as a primary limitation. The sweep in Figure~\ref{fig:dose_response} closes this for the visual modality: multiple metrics move monotonically with dose (Spearman/Mann--Kendall, BH-FDR), several admit sigmoid $E_{\max}$/Hill fits with a resolved activation threshold, and effect sizes are large. The residual limitations are now narrower and honest: (i) these curves are established on one distilled latent-diffusion model (SDXL-Turbo) and require replication on the language model and on non-distilled diffusion backbones; and (ii) the fitted threshold is only as sharp as the dose grid---we report it as a change-point interval rather than a point estimate where its confidence interval spans a grid step.
